<template>
  <b-form-group :label-cols="(columnLabel) ? labelCols : 0" :label="$t(columnLabel)"
    ...
    class="pf-form-security-event-trigger" :class="{ 'mb-0': !columnLabel, 'is-focus': focus }">
    <b-input-group>
      <multiselect
        v-model="inputValue"
        v-bind="$attrs"
  ...
  computed: {
    inputValue: {
      get () {
        ...
        // Check if its already expanded from what we receive from the API
 ...
 */
.pf-form-security-event-trigger {
  .multiselect {
      min-height: auto;
      border-width: 1px;
